...
for label in ["Attentive", "Distractive"]:
    ...
    for roi_file in os.listdir(label_dir):
        ...
        roi_image = cv2.resize(roi_image, (64, 64))  # Resize the ROI to match the model input size
...
# Reshape the data to match the CNN input shape (num_samples, height, width, channels)
...
roc_curve_path = os.path.join(output_dir, "roc_curve_2DConv.png")
plt.savefig(roc_curve_path)

# Save the trained model for future predictions
model.save(os.path.join(output_dir, "lecture_quality_model.h5"))